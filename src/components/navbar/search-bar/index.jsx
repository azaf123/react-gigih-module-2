...
class SearchBar extends Component {
    ...
    getSpotify = () => {
        fetch('https://api.spotify.com/v1/search?q=' + this.state.search + '&type=track&limit=10&access_token=' + this.props.accessToken
        )
            .then(res => res.json())
            .then(data => {
                console.log(data);
        ...
        return (
            <div className={Style.container}>
                <form nameName={Style.search}>
                    ...
                    <button className={Style.button} type="submit" onClick={this.handleSubmit}>Search</button>
                </form>
                        ...
                        <tbody>
                            {this.state.data.map((song, index) => (
                                <tr>
                                    ...
                                    <td><SelectSong song={song} onClick={this.getSelected} /></td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
